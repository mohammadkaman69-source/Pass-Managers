...
class PdfExportService {
  ...
  void _appendIndependentRecordGroup(
    ...
    final contentLengths = <double>[];
    for (final column in pdfColumns) {
      var maxLen = column.name.trim().length.toDouble();
      for (final row in group) {
        final field = _fieldAtPosition(row, column.position);
        if (field == null) continue;
        final raw = row.values[field.id.toString()] ?? '';
        if (raw.length > maxLen) {
          maxLen = raw.length.toDouble();
        }
      }
      if (maxLen < 4) maxLen = 4;
      if (maxLen > 40) maxLen = 40;
      contentLengths.add(maxLen);
    }
    final totalLen = contentLengths.fold<double>(0, (a, b) => a + b);
    final columnWidths = <int, pw.TableColumnWidth>{
      for (var index = 0; index < pdfColumns.length; index++)
        index: pw.FlexColumnWidth(
          totalLen <= 0 ? 1.0 : contentLengths[index] / totalLen,
        ),
    };
