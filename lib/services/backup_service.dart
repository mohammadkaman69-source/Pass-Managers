...
class BackupService {
  BackupService({TreeRepository? repository, SecurityManager? securityManager})
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          // Clipboard is a convenience only; a successful backup must not be
          // reported as failed because a platform clipboard is unavailable.
        }
        return true;
      } finally {
  ...
  Future<BackupVerificationResult> verifyBackup({required String credential}) async {
    ...
    try {
      return await _verifyBytes(source, credential);
    } finally {
  ...
  Future<BackupVerificationResult> _verifyBytes(
    ...
    try {
      ...
      throw const BackupFormatException('فایل نسخه پشتیبان خراب یا نامعتبر است.');
    }
    ...
    if (format == 'pass_managers_encrypted_backup' &&
        version != null && version >= 1 && version <= 3) {
  ...
  Future<Map<String, dynamic>> _readDatabaseSnapshot(Database db) async {
    List<Map<String, dynamic>> copyRows(List<Map<String, Object?>> rows) {
      return rows.map((row) => Map<String, dynamic>.from(row)).toList(growable: true);
    }
    return {
  ...
  Map<String, dynamic> _normalizeSnapshot(Map data) {
    List<Map<String, dynamic>> listFor(String key) {
      ...
      return value.map<Map<String, dynamic>>((item) {
        ...
        for (final k in [
          'id', 'parent_id', 'table_item_id', 'row_id', 'field_id',
  ...
  void _verifyManifest(Map expected, Map<String, dynamic> snapshot) {
    ...
    for (final key in [
      'tree_items', 'table_rows', 'table_fields', 'table_values',
  ...
  Future<String> _encryptString(String value, Uint8List keyBytes) async {
    final box = await _aes.encrypt(utf8.encode(value), secretKey: SecretKey(keyBytes));
