...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _createBackup() async {
    ...
    if (!verified) {
      if (mounted) _showError('Master Password اشتباه است.');
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      _showError(saved ? 'نسخه پشتیبان با موفقیت ذخیره شد.' : 'ذخیره نسخه پشتیبان لغو شد.');
    } catch (error) {
      if (!mounted) return;
      _showError('خطا در ایجاد نسخه پشتیبان: $error');
    } finally {
  ...
  Future<void> _restoreBackup() async {
    ...
    try {
      ...
      await _loadItems();
      if (!mounted) return;
      _showError('نسخه پشتیبان با موفقیت بازیابی شد.');
    } on BackupCancelledException {
      // User cancelled the picker.
    } catch (error) {
      if (!mounted) return;
      _showError('خطا در بازیابی نسخه پشتیبان: $error');
    } finally {
      if (mounted) setState(() => _isBackupBusy = false);
    }
  }
  ...
  Future<void> _configureBiometric() async {
    ...
    if (!supported) {
      _showError('بیومتریک روی این دستگاه در دسترس نیست.');
      return;
    }
    ...
    if (enabled) {
      ...
      if (disable == true) {
        ...
        if (mounted) _showError('ورود بیومتریک غیرفعال شد.');
      }
      return;
    }
