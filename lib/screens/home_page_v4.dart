...
class _HomePageV4State extends State<HomePageV4> {
    ...
    final result = await showDialog<String>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: TextField(
          ...
          autofocus: true,
          obscureText: true,
          decoration: InputDecoration(labelText: mainPasswordLabel, border: const OutlineInputBorder()),
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx), child: Text(cancelLabel)),
          ElevatedButton(onPressed: () { if (controller.text.isNotEmpty) Navigator.pop(ctx, controller.text); }, child: Text(continueLabel)),
        ],
      ),
    );
    controller.dispose();
  ...
  Future<void> _createBackup() async {
    ...
    if (!mounted) return;
    if (!verified) { _message(AppStrings.wrongPassword(context)); return; }
    setState(() => _busy = true);
    try {
  ...
  Future<void> _restoreBackup() async {
    ...
    final confirmed = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: Text(restoreTitle), content: Text(restoreWarning),
        actions: [
  ...
  Future<void> _showRecoveryKey() async {
    ...
    await showDialog<void>(context: context, barrierDismissible: false, builder: (ctx) => AlertDialog(
      title: Text(title),
      content: SingleChildScrollView(child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
        Text(description), const SizedBox(height: 16),
        SelectableText(key, textDirection: TextDirection.ltr, style: const TextStyle(fontFamily: 'monospace', letterSpacing: 0.8)),
      ])),
      actions: [
        TextButton(onPressed: () async { await Clipboard.setData(ClipboardData(text: key)); if (!ctx.mounted) return; ScaffoldMessenger.of(ctx).showSnackBar(SnackBar(content: Text(copiedMessage))); }, child: Text(copyLabel)),
  ...
  Future<void> _configureBiometric() async {
    ...
    if (!mounted) return;
    if (!supported) { _message(AppStrings.biometricUnavailable(context)); return; }
    ...
    if (enabled) {
      ...
      final disable = await showDialog<bool>(context: context, builder: (ctx) => AlertDialog(
        title: Text(title), content: Text(question),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: Text(cancelLabel)),
          ElevatedButton(onPressed: () => Navigator.pop(ctx, true), child: Text(disableLabel)),
        ],
      ));
      if (disable == true) { await _biometricService.disable(); if (mounted) _message(AppStrings.biometricDisabled(context)); }
      return;
    }
  ...
  Future<void> _chooseLanguage() async {
    ...
    final selected = await showDialog<String>(context: context, builder: (ctx) => AlertDialog(
      title: Text(title),
      content: RadioGroup<String>(groupValue: current, onChanged: (value) { if (value != null) Navigator.pop(ctx, value); }, child: Column(mainAxisSize: MainAxisSize.min, children: [
  ...
  void _message(String message) { if (!mounted) return; ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(message))); }
  ...
  Widget build(BuildContext context) {
    return Stack(children: [
      ...
      PositionedDirectional(
        ...
        child: SizedBox(width: 48, height: 56, child: IconButton(tooltip: AppStrings.settings(context), onPressed: _showMenu, icon: const Icon(Icons.more_vert))),
      ),
      if (_busy) Positioned.fill(child: IgnorePointer(child: ColoredBox(color: Colors.black12, child: Center(child: Card(child: Padding(padding: const EdgeInsets.all(20), child: CircularProgressIndicator())))))),
    ]);
  }
}
